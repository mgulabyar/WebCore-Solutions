...
// generateMetadata ko async params ke liye update kiya
export async function generateMetadata({
  params,
}: {
  ...
  const post = allBlogPosts.find((p) => p.slug === slug) ?? allBlogPosts[0];
...
// generateStaticParams same rahega lekin return slug
export async function generateStaticParams() {
...
// Main Page Component ko 'async' kiya aur params ko 'await' kiya
export default async function Page({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params; // Yahan params ko await karna zaroori hai
  ...
  if (!post) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-white text-center">
        <h1 className="text-6xl font-bold text-[#0062D6]">404</h1>
        <p className="mt-4 text-xl text-slate-600 font-medium">
          Blog data not found for slug: <span className="text-red-500">{slug}</span>
        </p>
        <Link 
          href="/blog" 
          className="mt-6 rounded-full bg-[#0062D6] px-6 py-2.5 text-sm font-semibold text-white transition-all hover:bg-[#0052b6]"
        >
          Back to Articles
        </Link>
      </div>
