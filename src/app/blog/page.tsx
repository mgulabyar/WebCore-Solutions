...
function ArticleRowCard({ post }: { post: (typeof allBlogPosts)[0] }) {
  return (
    <article className="group flex flex-col gap-6 overflow-hidden rounded-xl border border-slate-200 bg-white p-2 shadow-[0_4px_16px_rgba(15,23,42,0.05)] transition-all duration-400 hover:-translate-y-1 hover:border-blue-100 hover:shadow-[0_20px_45_rgba(0,98,214,0.1)] sm:flex-row sm:p-6">
      <div className="relative h-56 w-full shrink-0 overflow-hidden rounded-xl sm:h-auto sm:w-72">
        <img
          src={post.image}
          alt={post.title}
...
export default function BlogPage() {
  ...
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const perPage = 5; // Real data zyada hai to per page limit barha di
