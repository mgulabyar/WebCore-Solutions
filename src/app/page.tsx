...
function TestimonialsSection() {
  ...
  // Reset position during render when breakpoint (cloneCount) changes
  ...
  useEffect(() => {
    const real = ((trackIndex - cloneCount) % totalSlides + totalSlides) % totalSlides;
  ...
  // Position of the visually centered card within the extended track
            ...
            >
              {extended.map((t, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`group rounded-2xl border bg-white p-6 shadow-sm transition-all duration-500 ease-out hover:border-blue-300 h-[220px] flex flex-col ${
                        isCenter
                          ? "border-blue-200 shadow-xl -translate-y-3 scale-[1.03]"
                          : "border-slate-200 translate-y-0 scale-100"
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                          <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-blue-600 text-[11px] font-semibold text-white">
                            {t.name.charAt(0)}
                          </div>
                          <h3 className="text-sm font-semibold text-slate-900">
                            {t.name}
