...
export function ServicesHero() {
  ...
  const headline = "Architecting Scalable Systems. Engineered With Precision. Every Single Layer.".split(" ");

  const capabilities = [
    { name: "Frontend", icon: LayoutGrid, color: "text-blue-600 border-blue-200 bg-blue-50" },
    { name: "Backend", icon: Cpu, color: "text-cyan-600 border-cyan-200 bg-cyan-50" },
    { name: "Databases", icon: Database, color: "text-indigo-600 border-indigo-200 bg-indigo-50" },
    { name: "DevOps", icon: Server, color: "text-emerald-600 border-emerald-200 bg-emerald-50" },
  ];
    ...
    >
      <style>{`
        ...
        .premium-tag-transition:hover {
          ...
          background-color: rgba(0, 98, 214, 0.06);
          border-color: rgba(0, 98, 214, 0.2);
        }
      `}</style>
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-3 text-center sm:px-6 lg:px-8">
            ...
            <motion.span
              ...
              className={
                word === "Layer." || word === "Systems."
                  ? "bg-linear-to-r from-[#0062D6] via-cyan-500 to-indigo-500 bg-clip-text text-transparent font-medium"
                  : ""
              }
            >
              {word}
        ...
        <motion.p
          ...
          className="mt-5 max-w-2xl mx-auto text-base text-slate-600 leading-relaxed font-normal"
        >
        ...
        >
          {capabilities.map((item) => (
            <span
              key={item.name}
              className={`premium-tag-transition inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-1.5 text-xs font-medium text-slate-600 cursor-default`}
            >
  ...
  return (
    ...
    >
      <style>{`
        ...
        .db-icon-box .db-icon-inner {
          transition: transform 0.6s cubic-bezier(0.4, 0, 0.2, 1);
        }

        .db-card:hover .db-icon-box {
          transform: scale(1.08) translateY(-2px);
        }

        .db-card:hover .db-icon-inner {
        ...
        .db-card:hover .db-pulse-dot {
          animation: db-pulse 1.4s ease-out infinite;
        }

        @keyframes db-pulse {
          0% { box-shadow: 0 0 0 0 var(--skill-color); }
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {databaseSkills.map((skill, idx) => (
            ...
            >
              ...
              <div className="db-icon-box mb-6 flex h-14 w-14 items-center justify-center rounded-full border shadow-sm"
                style={{
              ...
              <div className="mt-7 flex items-center justify-between border-t border-neutral-100 pt-4">
                ...
                <span
                  className="db-pulse-dot h-2 w-2 rounded-full"
                  style={{
